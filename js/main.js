...
$(window).scroll(function () {
	...
	if (scrollTop >= (dh - wh) || scrollTop >= 600) {
 ...
 * AJAX Functions
 */
$(document).on('click', '.mywork-load-more-post:not(.loading)', function(event) {
	event.preventDefault();
	var that = $(this);
	var page = that.data('page');
	var newPage = page + 1;
    var ajaxurl = that.data('url');
    var $grid = $('.grid__inner');
    ...
    $.ajax({
		...
		success : function( response ) {
            ...
            if (response.length > 0) {
                var el = $(response);// convert the response to a jquery object
                setTimeout(function () {
